$files = @(
  'c:\Users\Synz\Downloads\Personal website file\index.html',
  'c:\Users\Synz\Downloads\Personal website file\dashboard.html',
  'c:\Users\Synz\Downloads\Personal website file\receipt.html'
)
...
$css = @'
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .status-badge, .parts-list li, .step-node, .chat-toggle {
      border-radius: 8px;
    }
    ...
    .estimator-grid, .estimate-total-bar, .admin-panel, .repair-card, .empty-state, .all-repairs, .lookup-bar, .progress-tracker, .merchant-qr-manager, .qr-panel, .cash-panel {
    ...
    .admin-panel, .repair-card, .empty-state, .all-repairs, .lookup-bar, .progress-tracker {
      background: var(--bg-panel);
      border: 1px solid var(--border-light);
      padding: 1rem 1.1rem;
    }
...
foreach ($file in $files) {
  ...
  $content = [regex]::Replace($content, '(?s)<style>.*?</style>', "<style>`n$css`n  </style>", 1)
  [System.IO.File]::WriteAllText($file, $content, [System.Text.UTF8Encoding]::new($false))
}

Write-Host 'Updated style blocks.'
